// app/clientele/ClienteleContent.jsx
"use client";
...
export default function ClienteleContent() {
  return (
    <div className="px-4 sm:px-8 md:px-16 lg:px-24 py-16 bg-gradient-to-b from-gray-50 to-white antialiased">

      <div className="text-center mb-16">
        <h1 className="text-4xl md:text-5xl font-black text-red-800 tracking-tight mb-4">
          Our Valuable Clients
        </h1>
        <div className="h-1 w-20 bg-red-600 mx-auto rounded-full"></div>
        <p className="mt-4 text-gray-600 max-w-2xl mx-auto">
          Trusted by leading organizations across India and globally for over 49 years
        </p>
      </div>

      {/* Logo Grid - Size Badhi */}
      <motion.div
        initial="hidden"
        animate="visible"
        variants={container}
      ...
      >
        {clients.map((client, idx) => (
          <motion.div
            key={idx}
            variants={item}
            className="group relative bg-white p-6 rounded-2xl border border-gray-100 shadow-sm hover:shadow-xl hover:border-red-200 transition-all duration-300 flex flex-col items-center justify-center overflow-hidden h-48"
          >
            <div className="relative w-full h-24 flex items-center justify-center transform group-hover:scale-105 transition-all duration-300">
              <Image 
                src={client.logo} 
            ...
            <div className="absolute inset-x-0 bottom-0 bg-red-950/95 text-white text-xs font-semibold p-2 text-center opacity-0 group-hover:opacity-100 transition-all duration-300 transform translate-y-2 group-hover:translate-y-0 leading-tight">
              {client.name}
            </div>
          </motion.div>
        ))}
              ...
              <div className="flex-1 p-6 flex flex-col justify-between">
                ...
                >
                  <div className="relative w-full max-w-[260px] mx-auto border-2 border-gray-100 rounded-xl overflow-hidden shadow-md bg-white p-1">
                    <Image
                      src={test.logo}
                      alt="Supplier Performance Certificate"
                      width={400}
                      height={500}
